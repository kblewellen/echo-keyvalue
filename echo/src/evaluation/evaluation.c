...
void *generic_store_create(int num_threads, int max_master_store){
  ...
  case USE_MASSTREE:
    rc =  masstree_table_open(&mt, MAX_THREADS, false, true);
    if(rc != 0){
      kp_die("thread_%lu: masstee_table_open() returned error=%d\n", tid, rc);
    }
    store = (void *) mt;
    break;
  ...
  case USE_LEVELDB:
    ...
    case USE_DISK:
      printf("ON PATH %s\n", DISK_PATH);
      rc = leveldb_client_start(&db, DISK_PATH, 0, false, use_durable_ldb);
      break; //SSD
    case USE_SSD:
      printf("ON PATH %s\n", SSD_PATH);
      rc = leveldb_client_start(&db, SSD_PATH, 0, false, use_durable_ldb);
      break; //SSD
    case USE_RAMDISK:
      printf("ON PATH %s\n", RAMDISK_PATH);
      rc = leveldb_client_start(&db, RAMDISK_PATH, 0, false, use_durable_ldb);      
      break; //RAMDISK
    default:
      kp_die("Request to use unknown Storage Platform\n");
      break; //NO STORAGE
    }
    if (rc != 0){
...
void *generic_worker_create(void *master_store, int max_local_store) {
  ...
  case USE_MASSTREE:
    mt = (masstree_t *)master_store;
    rc = masstree_client_start(mt, &mtc);
    if (rc != 0){
      kp_die("thread_%lu: masstree_client_start() returned error=%d\n",
             tid, rc);
    }
    store = (void*) mtc;
    break;
...
void generic_store_destroy(void *store){
  ...
  case USE_CASSANDRA:
    ...
    if(rc != 0){
      kp_die("thread_%lu: cassandra_stop() returned error=%d\n",
               tid, rc);
      return;
    }
    break;
  ...
  case USE_LEVELDB:
    ...
    if(rc != 0){
      kp_die("thread_%lu: leveldb_client_stop() returned error=%d\n",
               tid, rc);
      return;
    }
    break;
  ...
  case USE_MASSTREE:
    ...
    if (rc != 0){
      kp_die("thread_%lu: masstree_table_close() returned error=%d\n",
             tid, rc);
      return;
    }
    break;
...
void generic_worker_destroy(void *store){
  ...
  case USE_MASSTREE:
    mtc = (masstree_client_t*) store;
    rc = masstree_client_stop(mtc);
    if (rc != 0){
      kp_error("thread_%lu: masstree_client_stop() returned error %d\n",
               tid, rc);
      return;
    }
    break;
...
int generic_trans_end(void *store, void *trans){
...
#if 0  //threadpool version:
    uint64_t rc64;
    bool wait;
    void *conflict_list;
    rc64 = kp_local_commit(kv);
    if (rc64 == 0) {
      kp_warn("tried to commit an empty kvstore; this is unexpected...\n");
    } else if (rc64 == UINT64_MAX) {  // Initial commit failed
      kp_die("kp_local_commit returned error=%ju\n", rc64);
      #ifdef KP_ASSERT
      kp_die("kp_local_commit returned error=%ju\n", rc64);
      #endif
    } else {
      wait = true;
      conflict_list = NULL;
      state = kp_local_get_commit_result(kv, rc64, &conflict_list, wait);
      if (state == COMMIT_CONFLICT) { // Final commit failed with conflict
        retval = 1;
      } else if (state != COMMIT_COMMITTED) { //Final commit failed with error
        kp_error("kp_local_get_commit_result() returned error state=%s "
            "for commit %ju\n", commit_state_ext_to_string(state), rc64);
      }
    }
#endif
    break;
  ...
  case USE_LEVELDB:
    ...
    if(rc != 0){
      ...
      #ifdef KP_ASSERT
      kp_die("leveldb_client_stop_trans returned error=%d\n", rc);
      #endif
    }
    break;
  case USE_NOOP:
...
int generic_put(void *store, void * trans, const char *key, const char *value, const size_t size){
  ...
  case USE_LEVELDB:
    ...
    if(!trans)
      ...
      rc = leveldb_client_batch_put(db, trans, key, myval);
    if (rc != 0) {
      retval = -1;
      kp_die("leveldb_client_put failed\n");
      #ifdef KP_ASSERT
  ...
  case USE_MASSTREE:
    mtc = (masstree_client_t*)store;
    rc = masstree_client_put(mtc, key, value);
    if(rc != 0){
      kp_die("masstree_client_put failed\n");
    }
    break;
...
int generic_get(void *store, char *key, char **value, size_t* size){
  ...
  case USE_CASSANDRA:
    ...
    if (rc != 0) {
      ...
      if (rc == 1) {  //key not found
        ...
        kp_die(log_file, "cassandra_get returned error: %d\n", rc);
      }
    }
    break;
  #endif
  ...
  case USE_LEVELDB:
    ...
    if (rc != 0) {
      ...
      if (rc == 1) {  //key not found
        ...
        kp_die("leveldb_client_get returned error: %d\n", rc);
      }
    }
    break;
  ...
  case USE_MASSTREE:
    mtc = (masstree_client_t*) store;
    rc = masstree_client_get(mtc, key, (const char**) value, value_size);
    if(rc != 0 || value == NULL){
      retval = -1;
      if (value == NULL)
        kp_log(log_file, "masstree_client_get(%s) returned key not found\n",
               key);
      else{
        kp_log(log_file, "masstree_client_get(%s) returned error: %d\n", key, rc);
        kp_die( "masstree_client_get returned error: %d\n",rc);
      }
    }
    break;
...
int generic_delete(void *store, void *trans, char *key){
  ...
  case USE_MASSTREE:
    mtc = (masstree_client_t*) store;
    rc = masstree_client_delete(mtc, key);
    if (rc != 0){
      retval = -1;
      kp_error("leveldb_client_delete(%s) returned error: %d\n", key, rc);
    }
                     ...
                     bool *pause_before_start, int *delay)
{
  ...
  static struct option long_options[] =
  {
    ...
    {"masstree", no_argument, &WHICH_KEY_VALUE_STORE, 4},
...
int main(int argc, char *argv[]){
  ...
  printf("Now starting evaluation for key-value store: %s\n",
     ...
     WHICH_KEY_VALUE_STORE == 2 ? "LevelDB":
     WHICH_KEY_VALUE_STORE == 3 ? "Memcached":
     WHICH_KEY_VALUE_STORE == 4 ? "Masstree" : 
  ...
  if(num_threads >= 1){
    ...
    //Run Rampup threads over variety of percentages
    //    do_forked_run((void)(*rampup_percent_benchmark), num_threads, 1.0, 0.7, 0.01);
    //    do_forked_run((void)(*rampup_percent_benchmark), num_threads, 0.8, 0.7, 0.01);
    //    do_forked_run((void)(*rampup_percent_benchmark), num_threads, 0.6, 0.7, 0.01);
    //    do_forked_run((void)(*rampup_percent_benchmark), num_threads, 0.4, 0.7, 0.01);
    do_forked_run((void)(*rampup_percent_benchmark), num_threads, 0.2, 0.7, 0.01);
    //    do_forked_run((void)(*rampup_percent_benchmark), num_threads, 0.0, 0.7, 0.01);

    //Regular benchmark
    // fixed_generic_benchmark(num_threads, "generic", false);
    //Example changing percentages...
    //fixed_percent_benchmark(num_threads, 0.5, 0.7, 0.05);
    //Thread with partitioned datasets
