...
#define DISK_PATH  "/scratch/katelin/leveldb_client_name"
//#define DISK_PATH "/var/tmp/leveldb_client_name"
#define SSD_PATH "/scratch2/katelin/leveldb_client_name"
//#define SSD_PATH "/scratch/leveldb_client_name"
...
int generic_trans_end(void *store, void *trans){
  ...
  case USE_KP_KV_STORE:
    ...
    if (rc64 == 0) {
      ...
      if (state == COMMIT_CONFLICT) {
        retval = 1;
      } else if (state != COMMIT_COMMITTED) {
...
int split_latency_evaluation(int num_iterations, void* store){
  ...
  if (WHICH_KEY_VALUE_STORE == USE_CASSANDRA) {
    rc = individual_function_eval(num_iterations, 1, store, 
                                  &trans, CASSNOP, "cassandra-noop", 5, 0,
                                  false);    
  }
  #endif
...
void usage(char *progname)
{
  printf("USAGE: %s [--pause] [--delay=<secs>] [--kpvm-dram] [--leveldb-disk] "
         "[--leveldb-ramdisk] [--base] [--cache] <cpus> <iters> <ksize> <vsize> "
...
int main(int argc, char *argv[]){
  ...
  if(base){
    ...
    //    if(num_threads > 1){
      ...
      float tmp_put_prob = put_probability ;
      put_probability = 1;
      printf("0 GETS\n");
      ramp_up_threads(num_threads);
      ...
      //      fixed_zero_benchmark(num_threads);
      //      fixed_twenty_benchmark(num_threads);
      //      fixed_forty_benchmark(num_threads);
      //      fixed_sixty_benchmark(num_threads);
      //      fixed_eighty_benchmark(num_threads);
      //      fixed_hundred_benchmark(num_threads); 
